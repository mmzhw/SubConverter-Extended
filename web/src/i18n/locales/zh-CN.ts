export default {
  ...
  form: {
    ...
    inlineRulesToText: '批量编辑',
    inlineRulesToRows: '逐行编辑',
    inlineRulesInvalidLines: '有 {count} 行无法识别，不会写入链接；切回逐行编辑会丢弃这些行',
    rulePlacement: '规则位置',
    rulePlacementAppend: '末尾',
    rulePlacementPrepend: '置顶',
    rulePlacementPrependHint: '置顶的规则会排在最终规则表最前面，比远程配置自带的所有规则（含内网直连规则）更优先。',
  },
