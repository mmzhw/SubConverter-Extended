...
static bool expectAccepted(const std::string &input, bool expected) {
  bool value = !expected;
  if (!parseProxyProviderEnabled(input, value)) {
    std::cerr << "expected accepted proxy_provider enabled value: " << input
              << '\n';
    return false;
  }
  if (value != expected) {
    std::cerr << "unexpected enabled value for: " << input << '\n';
    return false;
  }
  return true;
}
...
static bool expectRejected(const std::string &input) {
  bool value = true;
  if (parseProxyProviderEnabled(input, value)) {
    std::cerr << "expected rejected enabled value: " << input << '\n';
    return false;
  }
  if (!value) {
    std::cerr << "rejected enabled value mutated output: " << input << '\n';
    return false;
  }
  return true;
}
...
int main() {
  bool ok = kDefaultProxyProviderEnabled;
